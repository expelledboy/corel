...
input_method_test_() ->
  ...
  [
   {"validate with atom",
    ?_assertEqual(ok, validate:this("test", string))
   },
   {"validate with tuple",
    ?_assertEqual(ok, validate:this("test", {string}))
   },
   {"validate from local function",
    ?_assertEqual(ok, validate:this("test", {is_test,fun is_test/1}))
   },
   {"validate from anonymous function",
    ?_assertEqual(ok, validate:this("test", {is_test,IsTest}))
   },
   {"list of validations of atom, tuple or function",
    ?_assertEqual(ok, validate:this("test", [{string},no_whitespace,{is_test,IsTest}]))
   },
   {"pass when given an empty list of validations",
    ?_assertEqual(ok, validate:this("test", []))
   }
  ].
...
reduction_test_() ->
  {setup,
   ...
   [
    {"validations can depend on other validations",
     fun test_no_whitespace_assumes_string/0
    },
    {"validations declared ealier are not included later",
     fun test_dups_not_reincluded/0
    }
   ]
  }.
...
test_pid_is_alive() ->
  ...
  ?assertEqual(true, validate:pid_alive(PID, [])),
  PID ! exit, timer:sleep(100),
  ?assertEqual(false, validate:pid_alive(PID, [])),
